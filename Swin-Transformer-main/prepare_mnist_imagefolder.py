...
def export_split(dataset, out_root, split_name, per_class):
    ...
    for image, label in dataset:
        ...
        image.convert("RGB").save(class_dir / f"{counts[class_name]:04d}.png")
        counts[class_name] += 1
        if len(counts) == 10 and all(v >= per_class for v in counts.values()):
            break
...
def main():
    ...
    parser.add_argument("--raw-root", default="data/mnist_raw")
    parser.add_argument("--out-root", default="data/mnist_imagefolder")
    ...
    val = MNIST(root=raw_root, train=False, download=True)
    ...
    print(f"Exported MNIST ImageFolder dataset to {out_root.resolve()}")
